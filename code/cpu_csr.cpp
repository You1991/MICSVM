...
Kernel_params kp;
int nPoints, nDimension, iLow, iHigh, iteration, max_index,elements;
...
void performTraining(){	
  ...
  in(parameterA,cost,tolerance,epsilon)\
  in(data:length(elements) alloc_if(1))\
  in(data_index:length(elements) alloc_if(1))\
  in(row_index:length(nPoints+1) alloc_if(1))\
  ...
  nocopy(lowKernel:length(nPoints) alloc_if(1))
  {
	while(bLow > bHigh + 2*tolerance){	  
	  int i;	  
	  #pragma omp parallel for schedule(ScheduleState) //num_threads(MY_THREADS)	
	  //#pragma simd //vectorlength(VectorLength)  
	  #pragma vector always aligned
		  ...
		  /*if(i==0&&iteration==1){
		  	  ...
		  	  printf("devF: %lf, alphaHighDiff: %lf, labels[iHigh]: %lf, highKernel[i]: %lf, alphaLowDiff: %lf, labels[iLow]: %lf, lowKernel[i]: %lf\n",devF[i], alphaHighDiff, labels[iHigh], highKernel[i], alphaLowDiff, labels[iLow], lowKernel[i]);
		  }	  */
	  	}
